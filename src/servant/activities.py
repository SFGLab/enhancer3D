...
spark = (
    ...
    .config("spark.executor.memory", int(os.getenv("SPARK_EXECUTOR_MEMORY", "4g")))
    .config("spark.driver.maxResultSize", int(os.getenv("SPARK_DRIVER_MAX_RESULT_SIZE", "1g")))
    .config("spark.authenticate", os.getenv("SPARK_AUTHENTICATE", "false").lower() == "true")
    .config("spark.authenticate.secret", os.getenv("SPARK_AUTHENTICATE_SECRET", ""))
    ...
    .config("spark.hadoop.fs.s3a.access.key", os.getenv("SPARK_S3A_ACCESS_KEY", ""))
    .config("spark.hadoop.fs.s3a.secret.key", os.getenv("SPARK_S3A_SECRET_KEY", ""))
    .config("spark.hadoop.fs.s3a.endpoint", os.getenv("SPARK_S3A_ENDPOINT", ""))
