...
def set_bin_number_for_models(region_start_ref, region_start_mod, genes, enhancers, df_enhancers_region_ref, df_genes_region_ref, df_enhancers_region_mod, df_genes_region_mod):
    ...
    full_enhancers_for_region['enh_center_position_mod'] = (full_enhancers_for_region['enh_start_mod'] +
                                                            full_enhancers_for_region['enh_end_mod']) // 2

    full_enhancers_for_region['enh_model_position_ref'] = (full_enhancers_for_region[
                                                               'enh_center_position_ref'] - region_start_ref) // 1000 + 1
    full_enhancers_for_region['enh_model_position_mod'] = (full_enhancers_for_region[
                                                               'enh_center_position_mod'] - region_start_mod) // 1000 + 1

    full_enhancers_for_region['enh_model_coloring_start_ref'] = (full_enhancers_for_region[
                                                                     'enh_start_ref'] - region_start_ref) // 1000 + 1
    full_enhancers_for_region['enh_model_coloring_end_ref'] = (full_enhancers_for_region[
                                                                   'enh_end_ref'] - region_start_ref) // 1000 + 1
    full_enhancers_for_region['enh_model_coloring_start_mod'] = (full_enhancers_for_region[
                                                                     'enh_start_mod'] - region_start_mod) // 1000 + 1
    full_enhancers_for_region['enh_model_coloring_end_mod'] = (full_enhancers_for_region[
    ...
    full_genes_for_region['gene_model_coloring_start_ref'] = (full_genes_for_region[
                                                                  'gene_start_ref'] - region_start_ref) // 1000 + 1
    full_genes_for_region['gene_model_coloring_end_ref'] = (full_genes_for_region[
                                                                'gene_end_ref'] - region_start_ref) // 1000 + 1
    ...
    full_genes_for_region['gene_model_coloring_start_mod'] = (full_genes_for_region[
                                                                  'gene_start_mod'] - region_start_mod) // 1000 + 1
    full_genes_for_region['gene_model_coloring_end_mod'] = (full_genes_for_region[
                                                                'gene_end_mod'] - region_start_mod) // 1000 + 1
...
def process_row(row_gene, coordinates_array_ref, coordinates_array_mod, region_chr_ref, region_start_ref, region_end_ref, region_chr_mod, region_start_mod, region_end_mod):
    distances_gene_enh = find_loci_distance_opt(
        row_gene['gene_model_position_ref'], row_gene['enh_model_position_ref'], coordinates_array_ref)
    distances_gene_enh_mut = find_loci_distance_opt(
        row_gene['gene_model_position_mod'], row_gene['enh_model_position_mod'], coordinates_array_mod)
    ...
    return [region_chr_ref, region_start_ref, region_end_ref, region_chr_mod, region_start_mod,
            region_end_mod] + list(row_gene) + [avg_dist_ref, avg_dist_mut, avg_dist_sub, mwh_pvalue, mwh_statistic,
...
def run_calculate_distance(c, p, region_ref, region_mod, models_in_ensemble, data_dir_path):
    ...
    genes_bed_mod = BedTool.from_dataframe(genes.loc[genes['gene_affected_by_svs'].isna()].reset_index()[
                                               ['gene_chr_mod', 'gene_start_mod', 'gene_end_mod',
                                                'index']].astype({'gene_start_mod': 'int32', 'gene_end_mod': 'int32'}))
    ...
    enhancers_bed_ref = BedTool.from_dataframe(enhancers.loc[enhancers['enh_affected_by_svs'].isna()].reset_index()[
                                                   ['enh_chr_ref', 'enh_start_ref', 'enh_end_ref', 'index']])
    enhancers_bed_mod = BedTool.from_dataframe(enhancers.loc[enhancers['enh_affected_by_svs'].isna()].reset_index()[
                                                   ['enh_chr_mod', 'enh_start_mod', 'enh_end_mod',
                                                    'index']].astype({'enh_start_mod': 'int32', 'enh_end_mod': 'int32'}))
    ...
    full_genes_for_region['gene_TSS_pos'] = np.where(
        ...
        full_genes_for_region['gene_start_ref'], full_genes_for_region['gene_end_ref'])
    ...
    df_results = pd.DataFrame(columns=['region_chr_ref', 'region_start_ref', 'region_end_ref', 'region_chr_mod',
                                       'region_start_mod', 'region_end_mod'] + list(df_filtered.columns) + [
                                          'average_dist_ref', 'average_dist_mut', 'average_dist_sub', 'mwh_pvalue',
                                          'mwh_statistics', 'number_bins_ref', 'number_bins_mod'])
    ...
    df_results = pd.DataFrame([r for r in results if r is not None],
                              columns=['region_chr_ref', 'region_start_ref', 'region_end_ref', 'region_chr_mod',
                                       'region_start_mod', 'region_end_mod'] + list(df_filtered.columns) + [
                                          'average_dist_ref', 'average_dist_mut', 'average_dist_sub', 'mwh_pvalue',
                                          'mwh_statistics', 'number_bins_ref', 'number_bins_mod'])
    ...
    df_results[['gene_chr_mod', 'gene_start_mod', 'gene_end_mod', 'gene_model_coloring_start_mod',
                'gene_model_coloring_end_mod',
                'gene_hgnc_ID']].drop_duplicates().to_csv(f'{data_dir_path_to_results}/genes_beads_annotation_mod.tsv', sep='\t', index=False)
